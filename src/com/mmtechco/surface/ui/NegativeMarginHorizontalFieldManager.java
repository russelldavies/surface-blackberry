...
/**
 * A basic vertical field manager that supports negative vertical margins It
 * also supports horizontal style bits.
 */
public class NegativeMarginHorizontalFieldManager extends Manager {
	...
	protected void sublayout(int maxWidth, int maxHeight) {
		...
		if (isStyle(Manager.HORIZONTAL_SCROLL)
				&& !isStyle(Manager.NO_HORIZONTAL_SCROLL)) {
		...
		for (int i = 0; i < numFields; ++i) {
			...
			marginLeft = calculateHorizontalMargin(prevMarginRight,
					field.getMarginLeft());
			marginRight = field.getMarginRight();

			layoutChild(field, widthAvail - marginLeft - marginRight,
					heightAvail - marginVertical);
