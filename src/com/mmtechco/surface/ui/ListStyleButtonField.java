...
public class ListStyleButtonField extends Field {
	...
	protected void paint(Graphics g) {
		// Left Bitmap
		if (_leftIcon != null) {
			g.drawBitmap(HPADDING, VPADDING, _leftIcon.getWidth(),
					_leftIcon.getHeight(), _leftIcon, 0, 0);
		}

		// Text
		try {
			g.pushRegion(_leftOffset, (getHeight() - _labelHeight) / 2,
					getWidth() - _leftOffset - _rightOffset, _labelHeight, 0, 0);
			_labelField.paint(g);
		} finally {
			g.popContext();
		}
		...
		if (_actionIcon != null) {
			g.drawBitmap(getWidth() - HPADDING - _actionIcon.getWidth(),
					...
					_actionIcon.getWidth(), _actionIcon.getHeight(),
					_actionIcon, 0, 0);
		}
	}
