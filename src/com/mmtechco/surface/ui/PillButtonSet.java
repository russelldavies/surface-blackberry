	...
	protected void sublayout(int maxWidth, int maxHeight) {
		...
		if (numChildren == 1) {
			...
			if (child instanceof PillButtonField) {
				((PillButtonField) child)
						.setDrawPosition(PillButtonField.DRAWPOSITION_SINGLE);
			}
		} else {
			child = getField(index);
			if (child instanceof PillButtonField) {
				((PillButtonField) child)
						.setDrawPosition(PillButtonField.DRAWPOSITION_LEFT);
			}
			for (index = 1; index < numChildren - 1; index++) {
				child = getField(index);
				if (child instanceof PillButtonField) {
					((PillButtonField) child)
							.setDrawPosition(PillButtonField.DRAWPOSITION_MIDDLE);
				}
			}
			child = getField(index);
			if (child instanceof PillButtonField) {
				((PillButtonField) child)
						.setDrawPosition(PillButtonField.DRAWPOSITION_RIGHT);
			}
		}
