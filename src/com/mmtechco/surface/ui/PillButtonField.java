...
public class PillButtonField extends BaseButtonField {
	...
	/**
	 * DRAWPOSITION_LEFT | DRAWPOSITION_RIGHT | DRAWPOSITION_MIDDLE |
	 * DRAWPOSITION_SINGLE Determins how the field is drawn (border corners)
	 */
	...
	public int getPreferredWidth() {
		return 2 * XPADDING + _buttonFont.getAdvance(_text); // not actually
																// used
	}
	...
	protected void paintBackground(Graphics g) {
		...
		try {
			if (_drawPosition == 0) {
				// Left
				drawButtonBackground(g, 0, 0, getWidth() + CORNER_RADIUS,
						getHeight(), backgroundColor, borderColor);
				drawSeparator(g, 0, 0, getWidth(), getHeight(), false, true);
			} else if (_drawPosition == 1) {
				// Right
				drawButtonBackground(g, -CORNER_RADIUS, 0, getWidth()
						+ CORNER_RADIUS, getHeight(), backgroundColor,
						borderColor);
				drawSeparator(g, 0, 0, getWidth(), getHeight(), true, false);
			} else if (_drawPosition == 2) {
				// Middle
				drawButtonBackground(g, -CORNER_RADIUS, 0, getWidth() + 2
						* CORNER_RADIUS, getHeight(), backgroundColor,
						borderColor);
				drawSeparator(g, 0, 0, getWidth(), getHeight(), true, true);
			} else {
				// Single
				drawButtonBackground(g, 0, 0, getWidth(), getHeight(),
	...
	private void drawButtonBackground(Graphics g, int x, int y, int width,
			int height, int backgroundColor, int borderColor) {
		...
		try {
			...
			g.drawRoundRect(x, _selected ? y : y + 1, width, _height - 1,
					CORNER_RADIUS, CORNER_RADIUS);
			g.setColor(0x000000); // Black lowlight
			g.drawRoundRect(x, _selected ? y + 1 : y, width, height - 1,
					CORNER_RADIUS, CORNER_RADIUS);
	...
	private void drawSeparator(Graphics g, int x, int y, int width, int height,
